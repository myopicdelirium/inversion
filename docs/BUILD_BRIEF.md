...
Phase 4 complete: sweep machinery (parallel across runs, byte-deterministic output) and heterogeneous individuals (per-agent time constants and attachment depth, drawn once at spawn, immutable, CLAUDE.md Amendment 2). Headline: trait spread does not move the aggregate death rate anywhere on the measured map, but the dead are a heavily biased sample (fast attachment clocks and deep initial attachments die). The toll belongs to the environment; the names on it belong to the individuals. Next: Phase 5 spec (the production phase diagram, plus survivor-composition maps as a second product).
